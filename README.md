...
<!-- prettier-ignore -->
> [!WARNING]
> Opal scripts run **full-trust, with no sandbox** — every `.js` file gets
> the JVM's full privileges (filesystem, threads, reflection into the whole
> client). Read a script before you run it, including the ones in this repo.
...
|---|---|
| [MilestoneToasts.js](core/MilestoneToasts.js) | Toast notifications for fall survival, low/full health, and sprint streaks — edge-triggered state tracking so each milestone fires once, not every tick. |
...
| Script | What it shows |
|---|---|
| [CombatHud.js](combo/CombatHud.js) | A combat-awareness HUD combining `esp` + `player` + `world` + `rotation`: target box, distance, an FOV-offset gauge, and your own crit/weapon status — and explains why there's no target health bar. |
